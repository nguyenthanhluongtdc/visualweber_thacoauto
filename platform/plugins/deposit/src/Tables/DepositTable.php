...
namespace Platform\Deposit\Tables;
...
class DepositTable extends TableAbstract
{
    ...
    public function __construct(DataTables $table, UrlGenerator $urlGenerator, DepositInterface $depositRepository)
    {
        ...
        if (!Auth::user()->hasAnyPermission(['deposit.edit', 'deposit.destroy'])) {
    ...
    public function ajax()
    {
        $data = $this->table
            ...
            ->editColumn('name', function ($item) {
                if (!Auth::user()->hasPermission('deposit.edit')) {
                    return $item->name;
                }
                return Html::link(route('deposit.edit', $item->id), $item->name);
            })
            ...
            ->addColumn('operations', function ($item) {
                return $this->getOperations('deposit.edit', 'deposit.destroy', $item);
            });
    ...
    {
        return $this->addDeleteAction(route('deposit.deletes'), 'deposit.destroy', parent::bulkActions());
    }
